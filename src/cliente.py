...
class Cliente:
    ...
    def __init__(self,nome,cpf,data_nascimento,email,endereco,telefone):
    ...
    def validar_cpf(self, cpf):               # Valida se o CPF apresenta o padrao esperado
        if not self.verifica_cpf(cpf):
            print(f"Erro: CPF {cpf} inválido")
            return None
        return cpf
    ...
    def calcula_soma(self, a, controle, dezena):   # Algoritmo parcial para validação de um CPF
        soma = dezena * 2                    # startando a soma ponderada
        for i in range(1 + controle, 10 + controle):
              resto = a % 10                 # armazena o ultimo digito da sequencia em questão
              soma += resto * i              # soma ponderada, aplicada sobre os digitos do CPF
              a = a // 10                    # Remove o último dígito da sequência analisada
        return soma                          # retorna a soma ponderada relacionada aos digitos do CPF em questão

    def verifica_cpf(self,cpf):              # funçao para verificar a veracidade do CPF apresentado
        cpf_num = ""
        for i in cpf:                        # loop "for" para representar o CPF como uma string contendo apenas numeros, excluindo pontos e traços
            if i.isdigit():
                cpf_num+=i
        if len(cpf_num) != 11:               # verifica se o CPF apresenta o número de dígitos padrão
            return False
        
        digito_verificador_um = 10*int(cpf_num[9])  # armazena o primeiro digito verificado do CPF
        digito_verificador_dois = int(cpf_num[10])  # armazena o segundo digito verificado do CPF
        
        controle = 1
        dezena = 0
        unidade = 0
        aux = self.calcula_soma(int(cpf_num[:9]), controle, dezena)
        resto = aux % 11
        if resto in [0, 1]:
            dezena=0
        else :
        ...
        controle = 2
        aux= self.calcula_soma(int(cpf_num[:9] + str(dezena)), controle, dezena)
        resto = aux % 11
        if resto in [0, 1]:
           unidade = 0 
        else: 
           unidade= 11 - resto
        if (digito_verificador_um + digito_verificador_dois) != ((10*dezena)+unidade):
           return False
        return True 
    ...
    def adicionar_conta(self, conta):       # Atrela uma nova conta ao seu respectivo cliente 
        ...
        return {"sucesso": True, "mensagem": f"Conta {conta.numero} adicionada ao cliente {self.nome}."}
